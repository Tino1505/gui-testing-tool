...
1. **`ACTION_LIST`**: Lưu trữ danh sách các hành động hợp lệ (navigate, click, input, select...). Đây là từ điển động từ của bạn. **Lưu ý**: Sheet này không bị khóa bảo vệ (unfrozen / unprotected) để người dùng có thể tự do thêm các hành động mới bằng cách nhập thêm hàng mới phía dưới.
...
3. **`ELEMENT`** (Object Repository): Là nơi lưu trữ "tọa độ" của các nút bấm, ô nhập liệu trên web. Nó giúp framework biết *nút Đăng nhập nằm ở đâu*, *ô chọn Bệnh viện là ô nào*.
   - **Lưu ý**: Cột `locator_type` (Cột B) có dropdown list giới hạn (`css, xpath, id, name, page`). Nếu nhập sai giá trị, Excel sẽ hiển thị cảnh báo lỗi và bắt buộc bạn phải chọn đúng giá trị trong danh sách này.
...
| | | | 2 | `check_status` | `dropdown_hospital` | *n.a* | `visible` |
| | | | 3 | `select_by_text` | `dropdown_hospital` | `Vinmec Times City` | *n.a* |
| | | | 4 | `input` | `txt_patient_name` | `Nguyen Van A` | *n.a* |
| | | | 5 | `click` | `btn_continue` | *n.a* | *n.a* |
| | | | 6 | `check_status` | `lbl_success_msg` | *n.a* | `visible` |
...
- **action**: Mở dropdown và chọn hành động (Ví dụ: `select` là chọn dropdown, `input` là gõ phím). Dữ liệu này map với code `ActionDispatcher`.
  - **Lưu ý**: Cột này có dropdown cảnh báo lỗi nếu nhập sai hành động, bắt buộc bạn phải chọn đúng giá trị có trong `ACTION_LIST`.
- **target**: Tên đối tượng bạn muốn thao tác.
  - **Lưu ý**: Cột này được cấu hình tự động hiển thị dropdown list lấy trực tiếp từ cột `element_id` (A2:A) ở sheet `ELEMENT`. Nếu nhập sai giá trị hoặc nhập phần tử chưa được khai báo ở sheet `ELEMENT`, Excel sẽ hiển thị cảnh báo lỗi và bắt buộc bạn phải chọn đúng giá trị hợp lệ. Khi code chạy, `ControlFactory` sẽ tìm target này trong sheet ELEMENT để lấy Locator.
  - **Quy tắc đặt tên Target (Prefix Mapping):** Hệ thống tự động nhận diện UI Control qua tiền tố:
    - TextBox/Input: `txt_`, `inp_`
    - Button: `btn_`
...
- **expected**: Kết quả mong đợi dùng cho các action `verify` hoặc `check_status`.
  - **Với Action `check_status`**, bạn có thể điền các trạng thái sau vào cột expected: `visible`, `hidden`, `exists`, `not_exists`, `enabled`, `disabled`. Nếu điền chuỗi khác, hệ thống sẽ mặc định là verify text (so sánh chuỗi hiển thị).
